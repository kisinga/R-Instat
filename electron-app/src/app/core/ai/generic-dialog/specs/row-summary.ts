...
registerBuilder('row-summary', (state) => {
  ...
  if (!df || !cols || cols.length < 2) return rSyntax().setBase('# Select at least 2 columns');

  const colList = cols.map(c => `\`${c}\``).join(', ');
  const naRmStr = naRm ? ', na.rm = TRUE' : '';
  return rSyntax().setBase(
...
const spec: DialogContract = {
  ...
  params: [
    ...
    p('func', 'enum', { required: true, enumValues: ['mean', 'sum', 'min', 'max', 'median', 'sd'], default: 'mean', label: 'Summary function' }),
  ...
  validate: (state) => {
    ...
    if (!Array.isArray(cols) || cols.length < 2) return 'Select at least 2 columns';
    return null;
  },
};
